...
class ImageLoadWorker(QThread):
    ...
    image_loaded = Signal(str, object)    # path, thread-safe QImage
    finished_loading = Signal(list)       # list of paths

    def __init__(self, image_dir, thumb_size=150, image_paths=None):
        super().__init__()
        self.image_dir = image_dir
        self._scan_directory = image_paths is None
        self.thumb_size = thumb_size
        self.image_paths = tuple(str(path) for path in (image_paths or ()))
    ...
    def run(self):
        if not self._scan_directory:
            png_files = sorted(self.image_paths)
        ...
        for i, fpath in enumerate(png_files):
            ...
            try:
                image = QImage(fpath)
                if not image.isNull():
                    thumb = self._create_thumbnail(image, self.thumb_size)
