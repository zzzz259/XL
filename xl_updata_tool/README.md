...
|---|---|
| 检查更新 | 检查并列出可下载的版本 |
| 导入 AS | 将已下载的 bundle 解析导出；仅勾选 Lua 且存在资源映射时只将命中的 AB 交给 AssetStudio；Lua 按版本写入 `output/lua/<版本>/`，成功后自动清理临时 Lua，并在最新已发布版本满足 Base 条件时自动解析角色；其他分类写入 `data/material/`（失败或取消时保留旧产物）|
| 刷新 | 刷新版本列表 |
| 作者 | 查看作者信息 |
